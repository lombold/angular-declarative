...
@Component({
  selector: 'ad-process-summary',
  imports: [],
  template: `
    @if (isSimpleValue()) {
    {{ input() }}
    } @else { @for (pair of inputAsKeyValue(); track pair.key) {
    <div>
...
export class ProcessSummaryComponent<In extends object> implements StepComponent<In, In> {
  ...
  protected readonly inputAsKeyValue = computed(() => this.mapInputToKeyValue(this.input()));
